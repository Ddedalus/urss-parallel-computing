...
library(magrittr)
library(devtools)
document(pkg = "./bspLib/") # compile the newest version of my package
library(bspLib)
```
Use a POETS graph to obtain a realistic distribution function:
```{r}
getwd()
sourceG <- readPOETSGraph("./external/asp/Networks/nodes8000.edges")
...
if(!file.exists("./data/from200to5000.rds")){      # to be automated, use precomputed values if available
  res <- runOnSizes(algoDistances, graphFromDist,
                  sizes, 300,
                  inputArgs = dfit, algArgs = c())
  saveRDS(res, file="./data/from200to5000.rds")
} else{
  res2 <- readRDS(file="./data/from200to5000.rds")
}
...
if(!file.exists("./data/bigres2.rds")){      # to be automated
  bigres2 <- runOnSizes(algoDistances, graphFromDist,
                    bigsizes, 150,
                    inputArgs = dfit, algArgs = c())
  saveRDS(bigres2, file = "./data/bigres2.rds")
} else{
  readRDS("./data/bigres2.rds")
}
