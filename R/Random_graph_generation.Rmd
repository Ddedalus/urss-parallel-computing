...
in_lnorm_fit <- fitdist(n5$in_deg, "lnorm3", start = c(meanlog=1, sdlog=0.1, threshold=0))
plot(in_lnorm_fit)
in_lnorm_fit
```
Nearly perfect! What about `n5$out_degree`?
```{r}
fit_lnorm_out <- fitdist(n5$out_deg, "lnorm3", start = c(meanlog=3, sdlog=0.1, threshold=-10))
plot(fit_lnorm_out)
```
