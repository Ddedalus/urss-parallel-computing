...
init_vals <- fitdist(n5$in_deg + 1, "lnorm")$estimate # use lognormal fit to obtain reasonable start conditions
in_lnorm_fit <- fitdist(n5$in_deg + 1, "lnorm3", start = c(init_vals, threshold=0))
plot(in_lnorm_fit)
in_lnorm_fit
shifted <- in_lnorm_fit$estimate
shifted["threshold"]  <- shifted["threshold"] - 1
qqplot(rlnorm3(10000, in_lnorm_fit$estimate) %>% round, rlnorm3(10000, shifted) %>% round)
```
