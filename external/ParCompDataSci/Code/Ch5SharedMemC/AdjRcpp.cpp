...
// arguments:
//    adjm:  the adjacency matrix (NOT assumed symmetric), 
//           1 for edge, 0 otherwise; note: matrix is overwritten 
//           by the function
//  return value: the converted matrix

// finds the hunk of rows this thread will process
...
{  
   ...
   {  int i,j,m;
      ...
      #pragma omp single
      {  
         cumul1s[0] = 0;  // cumul1s[i] will be tot 1s before row i of xadjm
         // now calculate where the output of each row in xadjm 
